...
function Navbar({ isHomePage = false }) {
  ...
  const scrollToSection = (sectionId) => {
    ...
    // Wait for menu to close (if it was open)
    setTimeout(() => {
  ...
  // Check if current page is home page for hash links
  ...
  const handleNavClick = (item, e) => {
    if (item.isHashLink && isCurrentPageHome()) {
      e.preventDefault();
      scrollToSection(item.hashId);
    }
    // For non-hash links, normal behavior
  };

  // Handle overlay menu click
  const handleOverlayNavClick = (item, e) => {
    if (item.isHashLink && isCurrentPageHome()) {
      e.preventDefault();
      e.stopPropagation();
      scrollToSection(item.hashId);
