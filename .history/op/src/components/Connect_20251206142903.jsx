...
const Connect = () => {
    useEffect(() => {
      // Shery.js initialization with proper timing
      const initShery = () => {
        if (window.Shery) {
          console.log("Initializing Shery.js magnet effect");
          window.Shery.makeMagnet(".magnet", {
            duration: 1,
            ease: "cubic-bezier(0.23, 1, 0.320, 1)"
          });
        } else {
          console.log("Shery.js not loaded yet, retrying...");
          setTimeout(initShery, 500);
        }
      };

      // Load Shery.js script if not already loaded
      if (!window.Shery) {
        const script = document.createElement('script');
        script.src = "https://cdn.jsdelivr.net/npm/sheryjs/dist/shery.min.js";
        script.onload = () => {
          console.log("Shery.js loaded successfully");
          initShery();
        };
        script.onerror = () => {
          console.error("Failed to load Shery.js");
        };
      ...
      // Also try initializing after a delay to ensure DOM is ready
      const timeoutId = setTimeout(initShery, 1000);

      return () => {
        clearTimeout(timeoutId);
        // Clean up if needed
        if (window.Shery && window.Shery.destroyAll) {
          window.Shery.destroyAll();
        }
      };
    }, []);
    ...
    <div className="h-[88vh] bg-black text-[#292929] overflow-hidden relative">
      <div className="h-full flex flex-col">
        ...
        <div className="flex items-center justify-center">
          {/* Center Button with magnet class */}
          <button className="magnet fixed-button px-12 py-6 bg-[#111111] text-white exo text-4xl hover:text-black hover:bg-white transition duration-500 font-medium rounded-full cursor-pointer">
            Let's Contact
          </button>
        </div>
      ...
      <style jsx>{`
        ...
        /* Shery.js magnet effect requires these CSS properties */
        .magnet {
          position: relative;
        ...
        /* Optional: Add some styles to make the effect more visible */
        .magnet:hover {
