...
function Navbar({ isHomePage = false }) {
  ...
  useEffect(() => {
    const handleMobileClickOutside = (event) => {
      // Only handle if mobile and menu is open
      if (!isMobile || !isMenuOpen) return;
      
      // Check if click is outside the menu content
      const menuContent = document.querySelector('.bg-white.rounded-t-3xl, .bg-white.rounded-2xl');
      const backdrop = document.querySelector('.bg-black\\/80');
      
      // If clicking on backdrop (grey area), close the menu
