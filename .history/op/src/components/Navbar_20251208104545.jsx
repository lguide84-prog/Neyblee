...
import { services } from '../data/data.js'; // Import your services data
...
function Navbar({ isHomePage = false }) {
  ...
  // Transform your services dataset into navbar structure
  const transformServicesData = () => {
    return services.map(service => ({
      title: service.name.toUpperCase(),
      href: `/services/${service.name.toLowerCase().replace(/\s+/g, '-')}`,
      hasSubDropdown: service.subservices && service.subservices.length > 0,
      subItems: service.subservices ? service.subservices.map(sub => ({
        title: sub.name.toUpperCase(),
        href: `/services/${service.name.toLowerCase().replace(/\s+/g, '-')}/${sub.name.toLowerCase().replace(/\s+/g, '-')}`,
        description: sub.description,
        // You can add image if needed
        image: sub.image
      })) : []
    }));
  };

  // Services data with proper structure
  const servicesData = {
    title: 'SERVICES',
    number: '02',
    ...
    href: '/services',
    hasDropdown: true,
    subServices: transformServicesData()
  };
  ...
  // Helper function to find service by title
  const findServiceByTitle = (title) => {
    return services.find(service => 
      service.name.toUpperCase() === title || 
      service.name.replace(' Services', '').toUpperCase() === title
    );
  };
              ...
              <div className="hidden md:flex items-center space-x-5 text-lg big">
                {menuItems.slice(0, 4).map((item, index) => (
                  ...
                  >
                    ...
                    {/* Main Dropdown for SERVICES - Using your actual data */}
                      ...
                      >
                        <div className="p-3">
                          {item.subServices.map((subItem, subIndex) => {
                            const serviceData = findServiceByTitle(subItem.title);
                            return (
                              <div 
                                key={subIndex} 
                              ...
                              >
                                ...
                                {/* Sub-Dropdown with actual subservices from your data */}
                                    ...
                                    <div className="p-3">
                                      ...
                                      {serviceData && (
                                        <div className="mb-2 px-3 py-2 border-b border-gray-800">
                                          <h4 className="text-white font-bold text-sm">{serviceData.name}</h4>
                                          <p className="text-gray-400 text-xs mt-1">{serviceData.description}</p>
                                        </div>
                                      )}
                                        ...
                                        >
                                          <div className="flex items-center">
                                            ...
                                            <div className="flex-1">
                                              <span className="block">{nestedItem.title}</span>
                                              <span className="block text-xs text-gray-400 mt-1">{nestedItem.description}</span>
                                            </div>
                                          </div>
                                        </a>
                                      ))}
                        ...
                        {item.subServices.map((subItem, subIndex) => {
                          ...
                          return (
                            <div key={subIndex} className="group">
                              ...
                              {subItem.hasSubDropdown && subItem.subItems.length > 0 && (
                                <div className="nested-menu ml-6 pl-4 border-l border-gray-200 space-y-1">
                                  {subItem.subItems.map((nestedItem, nestedIndex) => (
                                    <a
                                      key={nestedIndex}
                                      href={nestedItem.href}
                                    ...
                                    >
                                      <div className="flex items-start">
                                        ...
                                        <div>
                                          <span className="block">{nestedItem.title}</span>
                                          <span className="block text-sm text-gray-500">{nestedItem.description}</span>
                                        </div>
                                      </div>
                                    </a>
                                  ))}
