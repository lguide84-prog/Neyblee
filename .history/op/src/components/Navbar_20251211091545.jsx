...
function Navbar({ isHomePage = false }) {
  ...
  return (
    <>
      ...
      {/* MOBILE MENU OVERLAY - SIMPLE AND WORKING VERSION */}
      <div 
        ref={menuOverlayRef}
        className="fixed inset-0 z-40 hidden flex-col items-center justify-end md:hidden"
        style={{
                      ...
                      <div className="space-y-4">
                        {item.subServices.map((service, index) => (
                          <div key={service.id} className="pb-4 border-b border-gray-100 last:border-0">
                            {/* Service Header */}
                            <div 
                              className="cursor-pointer mb-2 hover:bg-gray-50 p-2 rounded transition-colors"
