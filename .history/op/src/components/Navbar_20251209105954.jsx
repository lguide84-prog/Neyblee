...
function Navbar({ isHomePage = false }) {
  ...
  return (
    <>
              ...
              <div className="hidden md:flex items-center space-x-5 text-lg big">
                {menuItems.slice(0, 4).map((item, index) => (
                  ...
                  >
                    ...
                    {/* Fixed Dropdown for SERVICES - ROWS LAYOUT */}
                    {item.hasDropdown && activeDropdown === item.title && (
                      <div 
                        className="absolute top-full right-20 mt-2 w-[850px] bg-white shadow-2xl rounded-2xl border border-gray-200 overflow-visible z-[100]"
                        onMouseEnter={() => handleMouseEnter(item.title)}
                        onMouseLeave={handleMouseLeave}
                      >
                        <div className="p-6">
                          
                          {/* Services List in Rows */}
                          <div className="space-y-6">
                            {item.subServices.map((service, serviceIndex) => (
                              <div 
                                key={service.id} 
                                className="relative pb-4 border-b border-gray-100 last:border-b-0 last:pb-0"
                              ...
                              >
                                ...
                                <a
                                  ...
                                  className="flex items-start group/service-row"
                                >
                                  <div className="w-2 h-1 rounded-full bg-yellow-500 mt-2 mr-4 flex-shrink-0"></div>
                                  <div className="flex-1">
                                    <div className="flex items-center justify-between">
                                      <h5 className="text-black font-bold text-base uppercase tracking-wide group-hover/service-row:text-blue-600">
                                        {service.name}
                                      </h5>
                                ...
                                {/* Subservices in ROWS - 3 ITEMS PER ROW */}
                                {service.subservices && service.subservices.length > 0 && activeSubDropdown === service.id && (
                                  <div 
                                    ref={subDropdownRef}
                                    className="mt-4 ml-6 bg-white shadow-lg rounded-lg border border-gray-200 overflow-hidden z-[200]"
                                  ...
                                  >
                                    <div className="p-4">
                                      ...
                                      <div className="mb-4 pb-3 border-b border-gray-100">
                                        <h6 className="text-black font-bold text-sm uppercase mb-1">{service.name} - Subservices</h6>
                                              ...
                                              <div key={i} className="grid grid-cols-3 gap-3">
                                                {rowItems.map((subservice, subIndex) => (
                                                  <a
                                                    ...
                                                    className="block p-3 bg-gray-50 hover:bg-white hover:shadow-sm transition-all duration-200 rounded border border-gray-100 hover:border-gray-300 group/subitem"
                                                  >
                                                    <div className="flex items-start mb-1">
                                                      <div className="w-1.5 h-1.5 rounded-full bg-blue-500 mt-1 mr-2 flex-shrink-0"></div>
                                                      <div className="flex-1">
                                                        <span className="block text-black font-medium text-xs mb-1 group-hover/subitem:text-blue-600">
                                                          {subservice.name}
                                                        </span>
                          ...
                          {item.subServices.find(service => service.id === 8) && (
                            <div className="mt-8 pt-6 border-t border-yellow-200">
                              <a
                                ...
                                className="flex items-center justify-between p-5 bg-gradient-to-r from-yellow-50 via-orange-50 to-yellow-50 hover:from-yellow-100 hover:via-orange-100 hover:to-yellow-100 transition-all duration-300 rounded-xl border-2 border-yellow-300 hover:border-yellow-400"
                              >
                                <div className="flex items-center">
      ...
      {/* Fullscreen Menu Overlay - ROWS LAYOUT */}
      <div 
        ref={menuOverlayRef}
            ...
            <div className="w-full space-y-1">
              {menuItems.map((item, index) => (
                ...
                >
                  ...
                  {/* Submenu for SERVICES in overlay - ROWS LAYOUT */}
                    ...
                    <div className="submenu overflow-hidden pl-6 mt-4">
                      <div className="border-l-2 border-gray-300 pl-6">
                        ...
                        <div className="space-y-6 mb-8">
                          {item.subServices.map((service, serviceIndex) => (
                            <div key={service.id} className="pb-4 border-b border-gray-100 last:border-b-0 last:pb-0">
                              {/* Service Row */}
                              <div 
                                className="cursor-pointer group/service-row mb-3"
                                onClick={(e) => handleServiceLinkClick(service.id, e)}
                              >
                                <div className="flex items-start">
                                  <div className="w-2 h-2 rounded-full bg-yellow-500 mt-2 mr-4 flex-shrink-0"></div>
                                  <div className="flex-1">
                                        ...
                                        rows.push(
                                          <div key={i} className="grid grid-cols-3 gap-2">
                                            {rowItems.map((subservice, subIndex) => (
                                              <div
                                                key={subservice.id}
                                                className="cursor-pointer p-2 bg-gray-50 hover:bg-white hover:shadow-sm transition-all duration-200 rounded border border-gray-100 hover:border-gray-300 group/subservice"
                                                onClick={(e) => handleSubserviceLinkClick(service.id, subservice.id, e)}
                                              >
                                                <div className="flex items-start">
                                                  <div className="w-1.5 h-1.5 rounded-full bg-blue-500 mt-1 mr-2 flex-shrink-0"></div>
                                                  <div className="flex-1">
                                                    <span className="block text-black font-medium text-xs mb-1 group-hover/subservice:text-blue-600">
                                                      {subservice.name}
                                                    </span>
                                                    <span className="block text-gray-500 text-xs line-clamp-2">
                                                      {subservice.description}
