...
function Navbar({ isHomePage = false }) {
  ...
  // Handle overlay menu click - FIXED VERSION
  const handleOverlayItemClick = (item) => {
    if (item.hasDropdown) {
      ...
      // Use setTimeout to navigate after menu animation completes
      setTimeout(() => {
        navigate(item.href);
      }, 400); // Match this with your menu close animation duration
    }
  };
  ...
  // Handle direct link navigation from overlay
                          ...
                          {item.subServices.map((service, serviceIndex) => (
                            ...
                            >
                              ...
                              {service.subservices && service.subservices.length > 0 && activeSubDropdown === service.name && (
                                <div 
                                  ref={subDropdownRef}
                                  className="absolute left-full top-0 ml-1 -mt-20 mb-1 w-80 bg-black/95 backdrop-blur-lg rounded-xl shadow-2xl border border-gray-800 overflow-hidden z-[200]"
