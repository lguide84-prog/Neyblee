...
function Navbar({ isHomePage = false }) {
  ...
  const handleOverlayItemClick = (item) => {
    if (item.hasDropdown) {
      ...
      // For non-dropdown items (HOME, PROJECTS), navigate and close menu
      handleMenuClick();
  ...
  return (
    <>
              ...
              <div className="hidden md:flex items-center space-x-5 text-lg big">
                {menuItems.slice(0, 4).map((item, index) => (
                  ...
                  >
                    ...
                    {/* Fixed Dropdown for SERVICES using your dataset - UPDATED DESIGN */}
                    {item.hasDropdown && activeDropdown === item.title && (
                      <div 
                        className="absolute top-full right-20 mt-2 w-80 bg-white shadow-2xl rounded-2xl border border-gray-200 overflow-visible z-[100]"
                      ...
                      >
                        <div className="p-4">
                          ...
                          <div className="mb-3 pb-3 border-b border-gray-100">
                            <h4 className="text-black font-bold text-lg">OUR SERVICES</h4>
                            <p className="text-gray-600 text-sm mt-1">Premium digital solutions for your business</p>
                          </div>
                          ...
                          {item.subServices.map((service, serviceIndex) => (
                            ...
                            >
                              <a
                                ...
                                className="flex items-center justify-between px-4 py-3 text-gray-700 hover:text-black hover:bg-gray-50 transition-all duration-300 text-sm font-medium exo rounded-lg"
                              >
                                <div className="flex items-center">
                              ...
                              {/* Sub-Dropdown with actual subservices - UPDATED DESIGN */}
                              {service.subservices && service.subservices.length > 0 && activeSubDropdown === service.name && (
                                <div 
                                  ref={subDropdownRef}
                                  className="absolute left-full top-0 ml-1 w-80 bg-white shadow-2xl rounded-xl border border-gray-200 overflow-hidden z-[200]"
                                  ...
                                  <div className="p-4">
                                    ...
                                    {service.subservices.map((subservice, subIndex) => (
                                      <a
                                        ...
                                        className="block px-4 py-3 text-gray-700 hover:text-black hover:bg-gray-50 transition-all duration-300 text-sm font-medium exo rounded-lg mb-1 group/item"
                                      >
                                        <div className="flex items-start">
                                          <div className="w-1.5 h-1.5 rounded-full bg-blue-500 opacity-0 group-hover/item:opacity-100 transition-opacity duration-300 mr-3 mt-1.5"></div>
                                          <div className="flex-1">
                                            <span className="block text-gray-800 group-hover/item:text-black font-medium">{subservice.name}</span>
                          ...
                          {/* Combo Services (check if id 8 exists in your services array) */}
                          {item.subServices.find(service => service.id === 8) && (
                            <div className="mt-2 pt-3 border-t border-gray-100">
                              <a
                                href="/subservice/8/subservices"
                                className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-yellow-50 to-orange-50 text-gray-700 hover:text-black hover:bg-gray-50 transition-all duration-300 text-sm font-bold exo rounded-lg border border-yellow-100"
                              >
                                <div className="flex items-center">
                                  <div className="w-2 h-2 rounded-full bg-yellow-500 mr-3"></div>
                                  <span>COMBO SERVICES</span>
                                </div>
                                <span className="text-yellow-600 text-xs font-bold">SAVE 20%</span>
                              </a>
                            </div>
                          )}
                        </div>
      ...
      {/* Fullscreen Menu Overlay - UPDATED DESIGN */}
      <div 
        ref={menuOverlayRef}
            ...
            <div className="w-full space-y-1">
              {menuItems.map((item, index) => (
                ...
                >
                  ...
                  {/* Submenu for SERVICES in overlay - UPDATED DESIGN */}
                  {item.hasDropdown && activeOverlayItem === item.title && (
                    <div className="submenu overflow-hidden pl-6">
                      <div className="border-l-2 border-gray-300 pl-4 my-2 space-y-2">
                            ...
                            {service.subservices && service.subservices.length > 0 && (
                              ...
                              >
                                {service.subservices.map((subservice, subIndex) => (
                                  <div
                                    key={subservice.id}
                                    className="block py-2 text-lg text-gray-600 hover:text-black transition-colors duration-200 pl-4 cursor-pointer"
                                    onClick={(e) => handleSubserviceLinkClick(service.id, subservice.id, e)}
                                  >
                                    <div className="flex items-start">
                                      <span className="mr-2 text-yellow-500">›</span>
                                      <div>
                                        <span className="block font-medium">{subservice.name}</span>
                                        <span className="block text-sm text-gray-500 mt-1">{subservice.description}</span>
                                      </div>
                                    </div>
                                  </div>
                                ))}
                        ...
                        {/* Combo Services Special Item */}
                        {item.subServices.find(service => service.id === 8) && (
                          <div className="mt-4 pt-3 border-t border-yellow-200">
                            <div 
                              className="flex items-center justify-between cursor-pointer py-3 text-yellow-700 hover:text-yellow-800 text-xl lg:text-2xl font-bold exo transition-all duration-300 hover:pl-4 bg-yellow-50 px-4 rounded-lg"
                            ...
                            >
                              ...
                              <span className="text-sm bg-yellow-500 text-white px-2 py-1 rounded">SAVE 20%</span>
                            </div>
                          </div>
                        )}
                        
                        <div
                          className="block py-3 text-yellow-600 hover:text-yellow-800 text-lg lg:text-xl font-bold exo transition-all duration-300 mt-2 cursor-pointer"
