...
function Navbar({ isHomePage = false }) {
  ...
  useEffect(() => {
    // Initialize menu overlay with proper state
  ...
  useEffect(() => {
    const handleMobileClickOutside = (event) => {
      if (isMobile && isMenuOpen && menuOverlayRef.current) {
        const menuContent = menuOverlayRef.current.querySelector('.bg-white');
        const backdrop = menuOverlayRef.current.querySelector('.bg-black\\/80');
        
        // If clicking on backdrop, close the menu
        if (backdrop && backdrop.contains(event.target)) {
          handleMenuClick();
        }
        
        // Also close if clicking on menu button while menu is open
  ...
  return (
    <>
      ...
      >
        {/* Backdrop - Click here to close menu */}
        <div 
          className="absolute inset-0 bg-black/80 backdrop-blur-sm"
          onClick={handleMenuClick}
        />
        
        {/* Menu Content - Click inside doesn't close */}
        <div 
          className="absolute inset-x-0 bottom-0 top-20 flex flex-col bg-white rounded-t-3xl md:rounded-none md:relative md:inset-auto md:w-96 md:ml-auto md:mr-4 md:my-4 md:rounded-2xl"
          onClick={(e) => e.stopPropagation()}
        >
          {/* Close Button */}
          <button 
            onClick={handleMenuClick}
            className="absolute right-4 top-4 z-10 w-10 h-10 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center transition-colors"
          >
              ...
              {menuItems.map((item, index) => (
                <div key={index} className="menu-item">
                  ...
                  {item.hasDropdown && activeOverlayItem === item.title && (
                    <div className="submenu pl-4 mt-2 mb-4 border-l-2 border-gray-300">
