...
function Navbar({ isHomePage = false }) {
  ...
  const handleMenuClick = () => {
    ...
    if (!wasOpen) {
      ...
      if (menuContentRef.current) {
        menuContentRef.current.style.transform = 'translateY(0)';
      ...
      if (menuContentRef.current) {
        menuContentRef.current.style.transform = 'translateY(100%)';
  ...
  return (
    <>
            ...
            <div className='flex items-center gap-5 relative' ref={dropdownRef}>
              ...
              <div className="md:hidden">
                <button 
                  ...
                  className="px-4 py-2 text-white text-md font-medium rounded-full transition-all duration-300 shadow-lg hover:shadow-xl magnet bg-blue-600"
                >
      ...
      <div 
        ref={menuOverlayRef}
        className="fixed inset-0 z-40 hidden flex-col justify-end"
        style={{
                ...
                <div key={index} className="border-b border-gray-100 pb-4 last:border-0">
                  ...
                  >
                    <div>
                      ...
                      <p className="text-gray-500 text-sm">({item.number})</p>
                    </div>
                    
                    {item.hasDropdown && (
                      <span className={`text-2xl transform transition-transform duration-300 ${activeOverlayItem === item.title ? 'rotate-90' : ''}`}>
                        ›
                      </span>
                    )}
                  </div>
                  ...
                  {item.hasDropdown && activeOverlayItem === item.title && (
                    <div className="mt-4 pl-4 border-l-2 border-gray-300">
                      <h4 className="text-lg font-semibold text-gray-800 mb-3">Our Services</h4>
                      ...
                      <div className="space-y-3">
                        {item.subServices.map((service, index) => (
                          <div key={service.id} className="mb-4">
                            {/* Service Header */}
                            <div 
                              className="cursor-pointer mb-2 p-2 hover:bg-gray-50 rounded"
                              onClick={(e) => handleServiceLinkClick(service.id, e)}
                            >
                              <div className="flex items-center mb-1">
                                <div className={`w-3 h-3 rounded-full ${index % 5 === 0 ? 'bg-blue-500' : index % 5 === 1 ? 'bg-green-500' : index % 5 === 2 ? 'bg-purple-500' : index % 5 === 3 ? 'bg-orange-500' : 'bg-pink-500'} mr-3`}></div>
                                <h5 className="text-base font-bold text-gray-900 flex-1">{service.name}</h5>
                              </div>
                              ...
                              <div className="ml-6">
                                <div className="grid grid-cols-2 gap-2 mt-2">
                                  {service.subservices.map((subservice) => (
                                    <div
                                      key={subservice.id}
                                      className="cursor-pointer p-2 bg-gray-50 hover:bg-blue-50 rounded border border-gray-200 hover:border-blue-300 transition-colors"
