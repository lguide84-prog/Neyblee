...
function Navbar({ isHomePage = false }) {
    ...
    if (!wasOpen) {
      ...
      if (menuOverlayRef.current) {
        ...
        if (menuContainer) {
          ...
          // Animate overlay background first
  ...
  // Clean up animation ref when component unmounts
  useEffect(() => {
    return () => {
      isAnimatingRef.current = false;
    };
    ...
    <>
      ...
      <div 
        ...
        style={{
          display: 'none',
          opacity: 0,
          pointerEvents: isAnimatingRef.current ? 'none' : 'auto'
        }}
      >
        {/* Backdrop */}
        <div 
        ...
        {/* Menu Content */}
        <div className="absolute inset-x-0 bottom-0 top-20 flex flex-col bg-white rounded-t-3xl md:rounded-none md:relative md:inset-auto md:w-96 md:ml-auto md:mr-4 md:my-4 md:rounded-2xl">
          {/* Close Button */}
          <button 
            onClick={handleMenuClick}
