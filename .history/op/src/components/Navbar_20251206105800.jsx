...
function Navbar({ isHomePage = false }) {
  ...
  return (
    <>
      ...
      <nav 
        className={`fixed top-6 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'shadow-xl bg-[#0E0E0E]/90 backdrop-blur-sm' : ''}`}
        style={{ padding: '0 24px' }}
      >
        <div className={`w-full px-6 py-4 lg:px-8 lg:py-4 ${isHomePage ? 'bg-transparent' : 'bg-[#0E0E0E]'} rounded-full transition-all duration-300 ${isScrolled ? 'scale-[0.98]' : ''}`}>
          <div className="flex items-center justify-between">
            ...
            <div className='flex items-center gap-5'>
              ...
              <div className="hidden md:flex items-center space-x-5 exo text-md">
                {navMenuItems.slice(0, 4).map((item, index) => (
                  <a 
                    key={index}
                    href="#" 
                    className={`text-gray-300 font-medium tracking-wide hover:text-white magnet transition-colors duration-300 bg-[#383838] px-5 py-1 rounded-full ${item.title === 'HOME' ? 'text-white' : ''} inline-flex items-center justify-center min-w-[100px]`}
                  >
              ...
              <button 
                ...
                className="px-5 py-2 text-white text-md font-medium rounded-full bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 transition-all duration-300 shadow-lg hover:shadow-xl magnet"
              >
