...
const Services = () => {
  ...
  const [imagePosition, setImagePosition] = useState({ top: 0, left: 0 }); // नई state
  ...
  const containerRef = useRef(null); // नया ref
  ...
  useEffect(() => {
    // Calculate and set initial image position
    const updateImagePosition = () => {
      if (containerRef.current) {
        ...
        setImagePosition({
          top: containerRect.top + scrollTop + containerRect.height / 2,
          left: containerRect.right - 350 // Adjust this value as needed
        });
      }
    };
    ...
    // Update position on scroll (for absolute positioning)
    const handleScroll = () => {
      if (containerRef.current && imageContainerRef.current && hoveredIndex !== null) {
        updateImagePosition();
      }
    };
  ...
  useEffect(() => {
    // Update image position when container mounts
  ...
  const handleMouseEnter = (index) => {
    ...
    // Update image position based on current scroll
    ...
    if (imageContainerRef.current) {
      ...
      // First make it visible and position it
  ...
  return (
    <div className="min-h-screen bg-white text-black" ref={servicesRef}>
      <section 
        className="pt-32 pb-20 lg:px-8 relative"
        ref={containerRef}
      >
        <div className="max-w-7xl mx-auto relative">
          ...
          {/* Floating Image - Fixed with absolute positioning inside container */}
          <div 
            ...
            style={{
              ...
              opacity: 0,
              willChange: 'transform, opacity, width, height' // Performance optimization
            }}
