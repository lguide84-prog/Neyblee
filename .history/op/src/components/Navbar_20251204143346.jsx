...
function Navbar({ isHomePage = false }) {
  ...
  return (
    <>
      ...
      <div 
        ref={menuOverlayRef}
        className="fixed inset-0 bg-black/80 backdrop-blur-md z-40 hidden items-end justify-end p-6"
      >
        {/* Menu Container - Image के according simple design */}
        <div 
          className="relative h-[85vh] w-[35vw] min-h-[600px] min-w-[400px] max-h-[95vh] max-w-[45vw] bg-gradient-to-br from-[#0A0A0A] via-[#111111] to-[#0A0A0A] rounded-l-3xl overflow-hidden shadow-2xl transform-gpu border-l border-gray-700/50"
          style={{
            transformOrigin: 'bottom right'
          }}
        >
          ...
          <button
            onClick={handleMenuClick}
            className="close-btn absolute top-8 right-8 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition-all duration-300 hover:rotate-90"
          >
            <span className="text-white text-2xl font-light">×</span>
          </button>

          {/* Main Content Area - Center aligned like image */}
          <div className="h-full flex flex-col justify-center items-center px-12 py-20">
            ...
            <div className="w-full max-w-md space-y-8">
              {menuOverlayItems.map((item, index) => (
                <a
                  key={index}
                  href="#"
                  className="menu-item group relative block w-full py-4"
                >
                  <div className="flex items-center justify-between">
                    {/* Title - Left side */}
                    <div className="relative overflow-hidden">
                      <h3 className="text-white text-5xl lg:text-6xl font-light tracking-tight opacity-90 group-hover:opacity-100 transition-all duration-500">
                        {item.title}
                      </h3>
                      {/* Underline animation */}
                      <div className="absolute bottom-0 left-0 w-0 h-0.5 bg-white group-hover:w-full transition-all duration-700"></div>
                    </div>
            ...
            {/* "GET IN TOUCH" Button - Bottom center like image */}
            <div className="mt-16 pt-8 border-t border-gray-700/30 w-full max-w-md">
              <button className="w-full py-5 bg-gradient-to-r from-transparent via-white/5 to-transparent border border-white/20 hover:border-white/40 rounded-lg group transition-all duration-500 hover:scale-[1.02]">
                <div className="flex items-center justify-center space-x-3">
                  <span className="text-white text-xl font-light tracking-widest group-hover:tracking-wider transition-all duration-300">
                    GET IN TOUCH
                  </span>
                  <span className="text-white opacity-0 group-hover:opacity-100 transition-all duration-500 transform group-hover:translate-x-2">
                    →
                  </span>
                </div>
                <p className="text-gray-400 text-xs mt-2 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
          ...
          {/* Decorative Elements - Minimal like image */}
          <div className="absolute -right-20 -top-20 w-64 h-64 rounded-full border border-white/5"></div>
          <div className="absolute -left-20 -bottom-20 w-64 h-64 rounded-full border border-white/5"></div>
        </div>
      </div>
    </>
  );
