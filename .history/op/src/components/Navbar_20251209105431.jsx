...
function Navbar({ isHomePage = false }) {
  ...
  return (
    <>
              ...
              <div className="hidden md:flex items-center space-x-5 text-lg big">
                {menuItems.slice(0, 4).map((item, index) => (
                  ...
                  >
                    ...
                    {/* Fixed Dropdown for SERVICES - IMPROVED DESIGN */}
                            ...
                            {item.subServices.map((service, serviceIndex) => (
                              ...
                              >
                                ...
                                <a
                                  ...
                                  className="block group/service"
                                >
                                  <div className="mb-2">
                                    <div className="flex items-center mb-1">
                                      <div className="w-2 h-2 rounded-full bg-yellow-500 mr-3"></div>
                                      <h5 className="text-black font-bold text-sm uppercase tracking-wide">{service.name}</h5>
                                    </div>
                                    <p className="text-gray-600 text-xs ml-5 line-clamp-2">{service.description}</p>
                                  </div>
                                </a>
                                
                                {/* Sub-Dropdown - FIXED STYLING */}
                                {service.subservices && service.subservices.length > 0 && activeSubDropdown === service.id && (
                                  <div 
                                    ref={subDropdownRef}
                                    className="absolute left-0 right-0 top-full mt-2 bg-white shadow-xl rounded-lg border border-gray-200 overflow-hidden z-[200]"
                                  ...
                                  >
                                    <div className="p-4">
                                      ...
                                      <div className="mb-4 pb-3 border-b border-gray-100">
                                        <h5 className="text-black font-bold text-base mb-1">{service.name}</h5>
                                        <p className="text-gray-600 text-xs">{service.description}</p>
                                      </div>
                                      
                                      {/* Subservices in 3 Columns - IMPROVED */}
                                      <div className="grid grid-cols-3 gap-3">
                                        {service.subservices.map((subservice, subIndex) => (
                                          <a
                                            key={subservice.id}
                                            href={`/subservice/${service.id}/detail/${subservice.id}`}
                                            className="block p-3 bg-gray-50 hover:bg-white hover:shadow-md transition-all duration-300 rounded-lg border border-gray-100 hover:border-gray-300 group/subitem"
                                          >
                                            <div className="flex items-start mb-1">
                                              <div className="w-1.5 h-1.5 rounded-full bg-blue-500 mt-1 mr-2 flex-shrink-0"></div>
                                              <div className="flex-1">
                                                <span className="block text-black font-medium text-xs mb-1 group-hover/subitem:text-blue-600">{subservice.name}</span>
                                                <span className="block text-gray-500 text-xs line-clamp-2">{subservice.description}</span>
                                              </div>
                                            </div>
      ...
      {/* Fullscreen Menu Overlay - IMPROVED DESIGN */}
      <div 
        ref={menuOverlayRef}
            ...
            <div className="w-full space-y-1">
              {menuItems.map((item, index) => (
                ...
                >
                  ...
                  {/* Submenu for SERVICES in overlay - IMPROVED */}
                    ...
                    <div className="submenu overflow-hidden pl-6 mt-4">
                      <div className="border-l-2 border-gray-300 pl-6">
                        ...
                        <div className="grid grid-cols-3 gap-4 mb-8">
                          {item.subServices.map((service, serviceIndex) => (
                            <div key={service.id} className="mb-4">
                              {/* Service Item */}
                              <div 
                                className="cursor-pointer group/service-item mb-3"
                                onClick={(e) => handleServiceLinkClick(service.id, e)}
                              >
                                <div className="flex items-center mb-1">
                                  <div className="w-2 h-2 rounded-full bg-yellow-500 mr-3"></div>
                                  <h5 className="text-black font-bold text-sm uppercase tracking-wide group-hover/service-item:text-blue-600">{service.name}</h5>
                                </div>
                                <p className="text-gray-600 text-xs ml-5 line-clamp-2">{service.description}</p>
                              </div>
                                    ...
                                    {service.subservices.map((subservice, subIndex) => (
                                      ...
                                      >
                                        <div className="flex items-start">
                                          <div className="w-1.5 h-1.5 rounded-full bg-blue-500 mt-1 mr-2 flex-shrink-0"></div>
                                          <div className="flex-1">
                                            <span className="block text-black font-medium text-xs mb-1 group-hover/subservice:text-blue-600">{subservice.name}</span>
                                            <span className="block text-gray-500 text-xs line-clamp-2">{subservice.description}</span>
                                          </div>
                                        </div>
                                      </div>
                        ...
                        {item.subServices.find(service => service.id === 8) && (
                          <div className="mb-6">
                            <div 
                              className="cursor-pointer p-4 bg-gradient-to-r from-yellow-50 to-orange-50 hover:from-yellow-100 hover:to-orange-100 transition-all duration-300 rounded-xl border border-yellow-200 hover:border-yellow-300 group/combo"
